...
`;
  ...
  try {
    ...
    if (apiError.message.includes("429") || apiError.message.toLowerCase().includes("quota") || apiError.message.toLowerCase().includes("limit")) {
      throw new Error("You are sending requests too fast! Google Gemini Free Tier has a rate limit of 15 requests per minute. Please wait 30 seconds and try sending again.");
    }
    throw apiError;
  }
