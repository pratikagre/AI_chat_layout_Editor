...
2. DO NOT include any comments (no // and no /* */).
3. DO NOT use literal newlines inside string values. You MUST escape them as \\n.
4. Ensure all commas are correctly placed between properties.
5. You MUST output MINIFIED JSON. Do NOT use indentation or line breaks in the JSON structure.
...
`;
  ...
  const chat = model.startChat({
    history: [
      ...
      {
        role: "model",
        parts: [{ text: "Understood. I will strictly output flawless, minified JSON without any comments, missing commas, or literal newlines." }]
      },
      ...geminiHistory
    ],
    generationConfig: {
  ...
  try {
    ...
    // Using JSON5 for bulletproof parsing against common LLM hallucinations
    return JSON5.parse(cleanText.trim());
  } catch (error) {
